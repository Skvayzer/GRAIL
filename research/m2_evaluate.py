...
and the four paired terrain controls, one simulator process at a time. Scene
...
def main():
    ...
    commands = []
    for name in ("validation", "stair_p1", "curb", "slope", "sitting"):
        cmd = [sys.executable, str(ROOT/"m2_train.py"), "--mode", "evaluate", "--resume", str(checkpoint),
               "--num-envs", "1", "--iterations", "16", "--wandb-mode", args.wandb_mode,
    ...
    for name, command in commands:
        ...
        completed = subprocess.run(command+["--execute"], cwd=ROOT.parent, text=True,
                                   stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        (output/(name+".log")).write_text(completed.stdout)
        records = [json.loads(line) for line in completed.stdout.splitlines()
                   if line.startswith('{"run":') and '"outputs_valid"' in line]
        record = dict(name=name, exit_code=completed.returncode, pipeline_passed=False)
        if records:
            ...
            if result_file.exists():
                ...
                record.update(pipeline_passed=completed.returncode == 0 and result["complete"]
